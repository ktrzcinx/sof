	...
	struct sof_ipc_pipe_comp_connect *connect)
{
	...
	if (!icd_source) {
		tr_err(&ipc_tr, "ipc_comp_connect(): source components already not exist, source_id = %u sink_id = %u",
	...
	if (!icd_sink) {
		tr_err(&ipc_tr, "ipc_comp_connect(): sink components already not exist, source_id = %d sink_id = %u",
